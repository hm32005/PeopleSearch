...
public class WikipediaParser {
	...
	public String parseImageLink(String text) {
		...
		int imstart1 = lowercased.indexOf("[[image");
		int imstart = lowercased.indexOf("image");
		if(imstart1 != -1 && (imstart1 < imstart || imstart == -1)){
			int start = lowercased.indexOf(":", imstart1);
			int end = lowercased.indexOf("|", start);
			String imageLink = null;
			if (start > 0 && end > 0 && end > start){
				imageLink = new String(text.substring(start+1, end));
				if(!(imageLink.toLowerCase().endsWith(".jpg") || imageLink.toLowerCase().endsWith(".jpeg"))){
					if(imageLink.toLowerCase().contains(".jpg"))
						imageLink = new String(imageLink.substring(0, imageLink.toLowerCase().indexOf(".jpg") + 4));
					else if(imageLink.toLowerCase().contains(".jpeg"))
						imageLink = new String(imageLink.substring(0, imageLink.toLowerCase().indexOf(".jpeg") + 5));
				}
				if(imageLink.toLowerCase().contains(".jpg") || imageLink.toLowerCase().contains(".jpeg"))
					return imageLink.trim();
			}
		}
		else if (imstart != -1 && ((text.indexOf("File:", imstart) != -1 && imstart < text.indexOf("File:", imstart)) || text.indexOf("File:", imstart) == -1)) {
			int start = lowercased.indexOf("=", imstart);
			int end = lowercased.indexOf("|", start);
			String imageLink = null;
			if (start > 0 && end > 0 && end > start){
				imageLink = new String(text.substring(start+1, end));
			...
			int end = lowercased.indexOf("|", start);
			if (end == -1 || end > lowercased.indexOf("]", start))
				end = lowercased.indexOf("]", start);
			String imageLink = null;
			if (start > 0 && end > 0 && end > start){
				imageLink = new String(text.substring(start+5, end));
				if(!(imageLink.toLowerCase().endsWith(".jpg") || imageLink.toLowerCase().endsWith(".jpeg"))){
					if(imageLink.toLowerCase().contains(".jpg"))
						imageLink = new String(imageLink.substring(0, imageLink.toLowerCase().indexOf(".jpg") + 4));
					else if(imageLink.toLowerCase().contains(".jpeg"))
	...
	public WikipediaParser(int idFromXml, String timestampFromXml, String authorFromXml, String ttl) throws ParseException {
